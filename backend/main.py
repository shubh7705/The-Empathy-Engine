...
from backend.app.emotion import detect_emotions, get_emotion_metadata, is_model_loaded
...
from backend.app.tts_engine import generate_audio
from backend.app.voice_mapper import (
    blend_voice_profile,
    build_ssml_preview,
    modify_text_blend,
)
...
async def list_voices():
    # Try live fetch from ElevenLabs account first.
    # Falls back to curated premade catalog if permission is missing or unconfigured.
    live_voices = fetch_voices_from_api()
    source = live_voices if live_voices else VOICE_CATALOG
    if live_voices:
        logger.info("Serving %d voices from ElevenLabs account.", len(live_voices))
    else:
        logger.info("Serving %d premade voices from catalog.", len(VOICE_CATALOG))
