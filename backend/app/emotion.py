...
Supports TOP-2 blended emotion detection for human-like expressions.
"""
...
def blend_emotions(primary_label: str, primary_score: float,
    ...
    return {
        "label": f"{meta['display'].lower()} with a hint of {sec_meta['display'].lower()}",
        "emoji": meta["emoji"],
        "color": meta["color"],
        "description": f"{meta['description']}, with some {sec_meta['description']}",
        "is_blended": True,
    }
